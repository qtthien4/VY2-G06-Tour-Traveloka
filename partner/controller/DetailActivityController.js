                ...
                re.query(queryimage, function(err, result){
                    ...
                    re.query(queryschedule, function(err, result){
                        ...
                        if(activity[0].Stt == true){
                            res.render('detailactivity',{activity: activity, image:image,schedule:schedule, stt:['Bật hoạt động']});
                        }else{
                            res.render('detailactivity',{activity: activity, image:image,schedule:schedule, stt:['Dừng hoạt động']});
                        }
            ...
            re.query(queryactivity, function(err, result){
                ...
                re.query(queryimage, function(err, result){
                    ...
                    re.query(queryschedule, function(err, result){
                        ...
                        res.render('detailactivity',{activity: activity, image:image,schedule:schedule });
                    })   
                    ...
                    re.query(queryimage, function(err, result){
                        ...
                        re.query(insertschedule, function(err, result){
                            ...
                            if(starttime > endtime){                            
                                ...
                                res.render('detailactivity',{activity: activity, image:image, err:['thời gian bắt đầu phải nhỏ hơn thời gian kết thúc'] });
                            }else{
                                var queryschedule = `select * from schedule where IdActivity = '${idActivity}'`
                                re.query(queryschedule, function(err, result){
                                    if(err) console.log(err)
                                   var  schedule = result.recordset
                                    ...
                                    res.render('detailactivity',{activity: activity, image:image, err:err, schedule:schedule });
                                })  
                            }
