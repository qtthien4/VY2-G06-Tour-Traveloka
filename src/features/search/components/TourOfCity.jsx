...
export default function TourOfCity({listCityofTour, handleOnclickTourSearch}) {
    ...
    return (
        <>
            ...
            <Box className={classes.boxItem} key={index} mb={3} onClick={()=>handleOnclickTourSearch(a.IdActivity)}>             
                <img height={200} width={300} className={classes.img} src={a.imageUrl} />
                <Box className={classes.content}>
                    <Typography className={classes.title} variant="h6">{a.ActivityName}</Typography>
                    <Box>
                        ...
                        <span className={classes.text} color="primary">{a.Location}</span>
                    </Box>
                    <Box>
